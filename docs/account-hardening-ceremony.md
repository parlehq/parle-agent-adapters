...
For the initial administrator ceremony, use a clean `parle-adapters` checkout at the reviewed commit. Run `pnpm install --frozen-lockfile` and `pnpm build` before the ceremony. In the separate terminal, invoke the helper from that checkout with `pnpm exec parle-hardening-secret <command>`. Do not substitute an unpinned downloaded package or an agent-authored wrapper. Issue #521 owns a generally distributed early-adopter entrypoint.

## Procedure

1. Run `parle_harden_account` with `action: "status"`.
2. In the separate terminal, run `parle-hardening-secret password`. Select `set` or `change`; all password prompts have disabled echo.
3. Run `parle_harden_account` with `action: "prepare"`, `confirmMutation: true`, and a reason.
4. Run `parle_harden_account` with `action: "enroll_totp"`, explicit confirmation, then run `parle-hardening-secret show-provisioning-qr` and scan the displayed QR directly into the human authenticator.
5. Run `parle-hardening-secret totp-code`, then `parle_harden_account` with `action: "confirm_totp"` and explicit confirmation. The recovery batch is written only to the protected local sink. It is never returned through the harness.
6. Move the recovery batch to the operator's protected destination. In the separate terminal run `parle-hardening-secret ack-recovery-stored`, typing its literal confirmation.
